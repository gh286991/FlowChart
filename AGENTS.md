...
- Node AI may read only the selected node title, its Markdown note, and ancestor titles needed for context.
- Node AI must not read sibling notes, descendant notes, embedded images, attachments, source URLs, or the entire map unless the user explicitly switches to whole-map AI.
- Whole-map AI may read node IDs, titles, parent relationships, branch sides, and sibling order.
- Whole-map AI must not read node notes, images, attachments, source URLs, or embedded file data.
- Whole-map structural changes must be previewed before they are applied.

## IME and editor rule

- Controlled inputs that support Chinese, Japanese, or Korean input MUST preserve composition events.
...
Before reporting that a change is ready:
